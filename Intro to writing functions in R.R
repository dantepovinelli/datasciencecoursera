...
above10<-function(vec) {
  use <- vec>10 #this creates a logical vector where the TRUE's will be used in the function output
  vec[use]
}
...
#to extend this, lets make it a two argument function which subsets above a given number x
...
#we can also give the function default values. This way R can run it even with unspecified arguments 
...
#to calculate mean value in each column of a dataframe or matrix (x); we will use a for loop
columnmean<-function(x){
  #first we make an empty numeric vector (to store col means) with length equal to the number of columns
  nc<-ncol(x)
  means<-numeric(length=nc)

  #the for loop will cycle through each column, assigning the col mean to the corresponding position in the vector we made
    for(i in 1:nc){
    means[i]<-mean(x[,i])
  }
  means
...
#Lets add that option now, with the default set to removing them from the analysis.
...
#we wrote the function to assume TRUE by default, so its identical with the argument left blank

#R's lexical scoping rules allows us to define free variables whithin another function;
#for example, this function below will take a power n and output a function that raises a number x to that power
